...
public class RankedLDA {
		...
		if (strTopic.isEmpty()) {
			...
			for (Story story : sM.getAllStories()) {
				...
				for (String strTemp : arrStory) {
					String tempOut = StoryParser.getInstance().RemoveStopWord(
							strTemp);
					if (!tempOut.isEmpty()
							&& !tempOut.equalsIgnoreCase(strTopic))
						strStory += tempOut + " ";
				}

				String strOut = story.GetID() + " " + "L" + nLabel + " "
						+ strStory;
			...
			for (String id : storyIDs) {
				...
				for (String strLine : vecStoryBody) {
					...
					for (String tempStr : arrStr) {
						String tempOut = StoryParser.getInstance()
								.RemoveStopWord(tempStr);
						if (!tempOut.isEmpty()
								&& !tempOut.equalsIgnoreCase(strTopic))
							strOut += tempOut + " ";
					}
					...
					try {
						out = new PrintWriter(new BufferedWriter(
								new FileWriter("storyOut_" + strTopic, true)));
						out.println(strOut);
					} catch (IOException ioe) {
						System.err.println("IOException: " + ioe.getMessage());
					} finally {
	...
	private void TopicIDF() {
		...
		try {
			...
			// br = new BufferedReader(new FileReader("data" + File.separator +
			// "word-mapping.txt"));
			...
			for (String key : sorted_map.keySet()) {
				...
				System.out.println(String.format("%s,%f,%f", key,
						mapTerms.get(key), Math.log10(fValue)));
			}
	...
	private void CalculatePMI() {
		...
		try {
			...
			while ((strLine = br.readLine()) != null) {
				...
				System.out.println(String.format("%d, %f, %f", n,
						GetPMIScore(vecAllWord), GetPMIScore(vecWord)));
				n++;
			}
			br.close();
		} catch (FileNotFoundException e1) {
	...
	private float GetPMIScore(Vector<String> vecStr) {
		...
		for (String str : vecStr) {
			for (int n = i; n < vecStr.size(); n++) {
				if (sM.getDocFrequency(str) == 0
						|| sM.getDocFrequency(vecStr.get(n)) == 0)
					continue;
				fRet += Math.log10(((float) sM.getDocCoFrequency(str,
						vecStr.get(n)) + 1)
						/ ((float) (sM.getDocFrequency(str) * (float) (sM
								.getDocFrequency(vecStr.get(n))))));
			}
			i++;
		}
	...
	private void LDAScoreEX() {
		...
		try {
			...
			while ((strLine = br.readLine()) != null) {
				...
				dTotalScore = TopicCompostion.getInstance().GetWeighting(
						vecStr, Integer.valueOf(strTopicID));
				vecAllWord.add(0, String.format("%f", dTotalScore));
				vecTopicTFIDF.add(vecAllWord);

				vecTopicToDocIDs.add(vecDocIDs);
				vecDocWeighting.add(TopicCompostion.getInstance().GetWeighting(
						vecDocIDs, Integer.valueOf(strTopicID)));
				...
				if (!vecWord.isEmpty()) {
					...
					// {
					// dTFIDF += TFIDFWorker.GetInstance().GetTFIDF(str,
					// vecDocIDs);
					// }
			...
			for (int k = 0; k < vecScoreOnly.size(); k++) {
				System.out.println(String.format("%f,%f,%f",
						vecScoreOnly.get(k), vecScoreOnly.get(k + 1),
						vecScoreOnly.get(k + 2)));
				k += 2;
			}
	...
	private void RandomTopicLDA() {
		...
		try {
			...
			for (int k = 0; k < vecScoreOnly.size(); k++) {
				...
				System.out
						.println(String.format("%f", vecScoreOnly.get(k + 1)));
	...
	private void CalculateTopicCoherence(Vector<String> vecWords) {
		...
		for (int i = 0; i < vecTempString.size() - 1; i++) {
			// System.out.print(vecTempString.get(i) + " " +
			// sM.getDocFrequency(vecTempString.get(i)) + " ");

			for (int j = i + 1; j < vecTempString.size(); j++) {
				dTopicCoherence += Math
						.log10(((double) sM.getDocCoFrequency(
								vecTempString.get(i), vecTempString.get(j)) + 1)
								/ ((double) sM.getDocFrequency(vecTempString
										.get(i)) * (double) sM
										.getDocFrequency(vecTempString.get(j))));
			}
		}
		// System.out.println();
		vecWords.add(
				0,
				String.format("[%f : %f]", dTopicCoherence, dTopicCoherence
						/ ((float) vecWords.size()
								* ((float) vecWords.size() - 1) / 2)));
		vecScoreOnly
				.add(dTopicCoherence
						/ ((float) vecWords.size()
								* ((float) vecWords.size() - 1) / 2));
